...
const url = 'https://api.nytimes.com/svc/books/v3/lists/current/hardcover-fiction.json?api-key=your Key'

const Book = () => {
    const [books, setBooks] = useState([])
    useEffect(() => {
        const fetchBooks = async () => {
            const res = await axios.get(`https://api.nytimes.com/svc/books/v3/lists/current/hardcover-fiction.json?api-key=9SMTJdMoH6o9GHXOegxRIEbB6QMrVjly`)
    ...
    return (
        <>
            ...
            <h1 className='font-bold text-center text-4xl py-5'>Book lists</h1>
            <section className='grid grid-cols-1 gap-10 px-5'>
                {books.map((book) => {
                    const { age_group, author, book_image, buy_links, description,  primary_isbn10, publisher, rank, title } = book
